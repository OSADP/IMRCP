...
public class Alerts extends BaseBlock
{
	...
	{
		m_nFileFrequency = oBlockConfig.optInt("freq", 86400000);
		m_oFormatter = new FilenameFormatter(oBlockConfig.optString("format", ""));
		m_nMaxForecast = oBlockConfig.optInt("maxfcst", 604800000);
		m_nHourOffsetStart = oBlockConfig.optInt("offsetstart", Integer.MIN_VALUE);
		m_nHourOffsetEnd = oBlockConfig.optInt("offsetend", Integer.MIN_VALUE);
		...
		try
		{
			...
			for (Network oNetwork : oWays.getNetworks()) // only generate alerts in existing networks
			{
				...
				for (int j = 0; j < nObsTypes.length; j++) // for each obs type
				{
					...
					ObsList oData = oStore.getData(nObsTypes[j], lStartTime, lEndTime, nBb[1], nBb[3], nBb[0], nBb[2], lValidTime); // get Obs
