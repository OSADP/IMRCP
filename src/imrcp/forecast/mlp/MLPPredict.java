...
public class MLPPredict extends MLPBlock
{
	...
	public void reset(JSONObject oBlockConfig)
	{
		...
		m_oFilenameFormat = new FilenameFormatter(oBlockConfig.optString("format", ""));
		m_nForecastMinutes = oBlockConfig.optInt("fcstmin", 120);
		m_bRealTime = oBlockConfig.optBoolean("realtime", false);
		m_sQueueFile = oBlockConfig.optString("queuefile", "");
		m_dBTol = oBlockConfig.optDouble("disttol", 160000);
		m_lPredStep = oBlockConfig.optLong("predstep", 900000);
		m_oQueue.clear();
	}
	...
	public void run()
	{
		if (checkAndSetStatus(1, 2)) // check if IDLE, if it is set to RUNNING and execute the block's task
		{
			try
			{
				...
				ObsList oSpeedData = oOv.getData(ObsType.SPDLNK, lStartTime, lEndTime, nBb[1], nBb[3], nBb[0], nBb[2], lEndTime); // get the necessary speed observations
				...
				ObsList oEventData = oOv.getData(ObsType.EVT, lStartTime, lEndTime, nBb[1], nBb[3], nBb[0], nBb[2], lEndTime); // get the necessary event data
	 ...
	 * @throws Exception
	 */
	protected void buildData(int nThread, long lStartTime, long lEndTime, ObsList oSpeedData, ObsList oIncidentData, Work oWorkObjs) throws Exception
	{
	...
	private void buildFirstData(long lEndTime)
	{
		try
		{
			...
			int[] nBb = oNetwork.getBoundingBox();
			ObsView oOv = (ObsView)Directory.getInstance().lookup("ObsView");
			ObsList oSpeedData = oOv.getData(ObsType.SPDLNK, lStartTime, lEndTime, nBb[1], nBb[3], nBb[0], nBb[2], lEndTime);
			ObsList oTemp = new ObsList();
			oTemp.ensureCapacity(oSpeedData.size());
			int nIndex = oSpeedData.size();
			int nMlpAContrib = Integer.valueOf("MLPA", 36);
			...
			ObsList oEventData = oOv.getData(ObsType.EVT, lStartTime, lEndTime, nBb[1], nBb[3], nBb[0], nBb[2], lEndTime);
			double dIncident = ObsType.lookup(ObsType.EVT, "incident");
			double dWorkzone = ObsType.lookup(ObsType.EVT, "workzone");
			double dFloodedRoad = ObsType.lookup(ObsType.EVT, "flooded-road");
			int nEventIndex = oEventData.size();
