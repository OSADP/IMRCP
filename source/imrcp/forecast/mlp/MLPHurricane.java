...
public class MLPHurricane extends BaseBlock
{
	...
	public void reset()
	{
		m_sNetworkId = m_oConfig.getString("network", "");
		m_oDest = new FilenameFormatter(m_oConfig.getString("dest", ""));
		m_oNhcFf = new FilenameFormatter(m_oConfig.getString("nhcfile", ""));
		m_sSvmInputFf = m_oConfig.getString("svmff", "");
		m_sSvmMarkov = m_oConfig.getString("svmmarkov", "");
		m_sRScript = m_oConfig.getString("script", "");
		m_sWorkingDir = m_oConfig.getString("dir", "");
		m_nThreads = m_oConfig.getInt("threads", 5);
		m_nPeriod = m_oConfig.getInt("period", 300);
		m_nOffset = m_oConfig.getInt("offset", 60);
		m_nHorizon = m_oConfig.getInt("horizon", 6);
		m_sOneshotFf = m_oConfig.getString("oneshot", "");
		m_sStormInfoFf = m_oConfig.getString("storminfo", "/dev/shm/imrcp-test/mlp/hurricane/%s.info");
	}
	...
	public boolean buildData(String sNhcFile)
	{
		try
		{
			...
			ImrcpObsResultSet oSpeedData = (ImrcpObsResultSet)((ObsView)Directory.getInstance().lookup("ObsView")).getData(ObsType.SPDLNK, lStart, lEnd, nNetBb[1], nNetBb[3], nNetBb[0], nNetBb[2], lRef);
...
//			oTempWays.add(oWayNetworks.getWayById(new Id("046b442edc790cd5b202e142f1d7aa7f")));
	...
	public class Delegate implements Runnable
	{
		 ...
		 */
		ImrcpObsResultSet m_oSpeedData;
		 ...
		 */
		Delegate(HurWork[] oWorks, String sOutput, ImrcpObsResultSet oSpeeds, int nRun)
		{
			m_oWorks = oWorks;
			m_sOutput = sOutput;
			...
			try
			{
				...
				try
				{
					...
					if (m_oDelegate.m_nRun == 0) // only need to create input for SVM model once
					{
						...
						while (lTime < m_oFile.m_lStartTime + 604800000) // always have at least 7 days of data
						{
							...
							if (oObs != null)
							{
								...
								if (nLatHur == 0)
									nLatHur = oObs.m_nLat1;
								if (nLonHur == 0)
									nLonHur = oObs.m_nLon1;
								if (nMaxWindSpeed == 0)
					...
					while (lTime <= m_oFile.m_lStartTime)
					{
						...
						ImrcpObsResultSet oSpeedData = new ImrcpObsResultSet();
						...
						for (int nHour = 0; nHour < 24; nHour++)
						{
							...
							if (oObs != null)
							{
								if (oObs.m_dValue == ObsType.lookup(ObsType.TRSCAT, "Major Hurricane") || oObs.m_dValue == ObsType.lookup(ObsType.TRSCAT, "Hurricane"))
									...
									nStatus[nHour] = 1;
								nLatHur[nHour] = oObs.m_nLat1;
								nLonHur[nHour] = oObs.m_nLon1;
							}
						}
						for (OsmWay oWay : m_oWays)
						{
