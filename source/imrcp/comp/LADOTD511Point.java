...
/**
 * Processes the rolling event file for points from the LADOTD's 511 system
 ...
 */
public class LADOTD511Point extends EventComp
{
	/**
		...
		throws Exception
	{
		...
		try (CsvReader oIn = new CsvReader(Files.newInputStream(oRollingFile)))
		{
			...
			while (oIn.readLine() > 0)
			{
				...
				String sUpdate = oIn.parseString(18);
				if (sUpdate.contains("."))
		...
		try
		{
				 ...
				 BufferedWriter oOut = new BufferedWriter(Channels.newWriter(Files.newByteChannel(oRollingFile, FileUtil.APPENDTO, FileUtil.FILEPERS), "UTF-8")))
			{
				...
				while ((nCol = oIn.readLine()) > 0)
				{
					...
					String sEndTime = oIn.parseString(17);
					int nIndex = Collections.binarySearch(m_oUpdateTimes, lSearch, oUpdateComp);
					if (nIndex < 0)
					{
					...
					if (nEventIndex < 0)
					{
						String sExt = oIn.parseString(22);
					...
					if (lUpdate[1] < lSearch[1]) // if the event has been updated
					{
						...
						oEvent.m_nLat1 = GeoUtil.toIntDeg(Double.parseDouble(sGeo.substring(nStart, nEnd)));
						nStart = nEnd + 1;
						oEvent.m_nLon1 = GeoUtil.toIntDeg(Double.parseDouble(sGeo.substring(nStart, sGeo.indexOf(")"))));
						
						String sRoadName = oIn.parseString(4);
						oEvent.m_sDir = oIn.parseString(5).substring(0, 1).toLowerCase();
						String sType = oIn.parseString(8);
						if (sType.compareTo("Roadwork") == 0)
							oEvent.m_sType = "workzone";
						else
							oEvent.m_sType = "incident";
						
						oEvent.m_sEventName = oIn.parseString(9) + " at " + sRoadName;
						String sStartTime = oIn.parseString(16);
						...
						oEvent.m_nLanesAffected = oIn.parseInt(27);
