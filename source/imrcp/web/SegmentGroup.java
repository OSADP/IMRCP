...
public class SegmentGroup
{
	...
	public SegmentGroup(JSONObject oGroup)
	{
		...
		JSONArray oPlowed = oGroup.getJSONArray("plowed");
		m_bPlowed = new boolean[oPlowed.length()];
		for (int nIndex = 0; nIndex < oPlowed.length(); nIndex++)
			m_bPlowed[nIndex] = oPlowed.getBoolean(nIndex);
		
		JSONArray oTreated = oGroup.getJSONArray("treated");
		m_bTreated = new boolean[oTreated.length()];
		for (int nIndex = 0; nIndex < oTreated.length(); nIndex++)
			m_bTreated[nIndex] = oTreated.getBoolean(nIndex);
