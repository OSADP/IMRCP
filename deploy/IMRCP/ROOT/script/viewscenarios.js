...
import {addSourceAndLayers, removeSourceAndLayers, pointToPaddedBounds, featureInSources, iconDivFromSprite, g_oLayers,
...
[
	{
		...
		[
			...
			['Flooded', '#39f', 30, 31],
			['Frost', '#fdafff', 13, 14],
			['Ice/Snow', '#00faff', 20, 21],
			['Slush', '#00faff', 21, 22],
			['Melting Snow', '#00faff', 22, 23],
			['Icing Rain', '#af1cff', 23, 24]
		],
		'valuemap': {3: 'Dry', 5: 'Wet', 12: 'Dew', 13: 'Frost', 20: 'Ice/Snow', 21: 'Slush', 22: 'Melting Snow', 23: 'Icing Rain', 30: 'Flooded'},
		'features': [],
		'layer': 
...
let g_oVectorSrcs = 
{
	'pccat' :
	{
		...
		[
			...
			{'id':'PCCAT9', 'source':'pccat', 'type':'fill', 'metadata':{'imrcp-range':'Snow - Heavy'}, 'source-layer':'PCCAT9', 'paint':{'fill-color':'#006'}},
...
async function initialize()
{
	...
	let oMap = new mapboxgl.Map({container: 'mapid', style: 'mapbox/light-v9.json', attributionControl: false, minZoom:4, maxZoom: 22, center: [-98.585522, 39.8333333], zoom: 4});
	g_oMap = oMap;
	...
	{
		oMap.addSource('areainsert', {'type': 'geojson', 'data': {'type': 'FeatureCollection', 'features': []}});
		oMap.addLayer({'id': 'areainsert', 'type': 'line', 'source': 'areainsert'}, 'road-number-shield');
