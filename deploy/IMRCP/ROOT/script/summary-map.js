...
{
	if (map.getSource('adcirc') !== undefined)
	{
		if (map.getLayer('adcirc') !== undefined)
		{
			map.removeLayer('adcirc');
			map.addLayer(g_oLayers['adcirc'], 'road-number-shield');
		}
	}
...
class TextControl
{
	...
	onAdd(map)
	{
		...
		this._container.innerHTML = `<button class="mapboxgl-ctrl-icon" disabled type="button" id="${this.sId}">${map.getZoom().toFixed(1)}</button>`;
...
async function initialize()
{
	...
	const map = new mapboxgl.Map({container: 'mapid', style: 'mapbox/light-v9.json', attributionControl: false,
		minZoom:4, maxZoom: 22, center: [lng, lat], zoom});
	...
	map.addControl(new TextControl('', 'imrcp-zoom-display'));
...
//		});
		...
		map.on('zoom', () => {$('#imrcp-zoom-display').html(map.getZoom().toFixed(1));});
		map.on('zoomend', () => {$('#imrcp-zoom-display').html(map.getZoom().toFixed(1));});
	});
